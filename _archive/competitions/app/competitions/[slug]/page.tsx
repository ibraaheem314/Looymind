...
export default function CompetitionDetailPage() {
  ...
  const handleDatasetDownload = async (datasetId: string) => {
    try {
      console.log('📥 Téléchargement de dataset:', datasetId)
      await supabase.rpc('increment_dataset_downloads', { dataset_id: datasetId })
              ...
              <div className="space-y-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Téléchargez les datasets pour participer à cette compétition
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    {datasetsLoading ? (
                      ...
                      <div className="text-center py-8 text-gray-600">
                        <p>Les datasets seront bientôt disponibles</p>
                        {competition.dataset_url && (
                          <Button className="mt-4" asChild>
                            <a href={competition.dataset_url} download>
                              <Download className="h-4 w-4 mr-2" />
                              Télécharger depuis le lien externe
                            </a>
                          </Button>
                        )}
                      </div>
                          ...
                          >
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-1">
                                <span className="text-sm font-semibold">{getFileTypeLabel(dataset.file_type)}</span>
                                <Badge variant="outline" className="text-xs">
                                  {formatFileSize(dataset.file_size)}
                                </Badge>
                              </div>
                              <p className="text-sm text-gray-600">{dataset.file_name}</p>
                              {dataset.description && (
                                <p className="text-xs text-gray-500 mt-1">{dataset.description}</p>
                              )}
                              <p className="text-xs text-gray-400 mt-1">
                                {dataset.download_count} téléchargement{dataset.download_count > 1 ? 's' : ''}
                              </p>
                            </div>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Métrique d'Évaluation</CardTitle>
                  </CardHeader>
                  <CardContent>
